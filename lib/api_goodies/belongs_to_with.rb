...
module APIGoodies
  module BelongsToWith
    ...
    def has_and_belongs_to_many_with(attribute_name, name, options = {})
      ...
      attribute_names = attribute_name.to_s.pluralize

      class_eval <<-CODE, __FILE__, __LINE__ + 1
        attr_writer :#{name}_#{attribute_names}
        before_validation :set_#{name}_from_#{attribute_names}

        alias_method :old_#{name}, :#{name}

        def #{name}
          if @#{name}_#{attribute_names}.nil?
            old_#{name}
          else
            set_#{name}_from_#{attribute_names}
          end
        end

        def #{name}_#{attribute_names}
          @#{name}_#{attribute_names} || #{name}.map(&:#{attribute_name})
        end

        private

        def set_#{name}_from_#{attribute_names}
          if @#{name}_#{attribute_names}
            other_class = association(:"#{name}").klass
            return if other_class.nil?
            self.#{name} = tmp = other_class.where(#{attribute_name}: @#{name}_#{attribute_names}).to_a
            @#{name}_#{attribute_names} = nil
            tmp
          end
        end
      CODE
